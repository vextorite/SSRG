...
#add job id param
    ...
    def constructMossShellCommand(self):
        print('unzipping')
        os.system(f"python3 /home/Vextorite/Documents/Capstone/ssrg-ndxsas021-hlnsan005-rmrsuv002/backend/SSRG/Jobs/MossBackendJobs/folderizer.py '{self.zipPath}' > folder_out.txt")
        ...
        if(self.directoryFormat==True) and (len(self.baseFile)>1):
            osCommand = f"perl /home/Vextorite/Documents/Capstone/ssrg-ndxsas021-hlnsan005-rmrsuv002/backend/SSRG/Jobs/MossBackendJobs/mossnet.pl -l {self.submissionLanguage} -d -b {self.baseFile} {subPathList} > myMossRun.txt"
        if(self.directoryFormat==True) and (len(self.baseFile)==0):
            osCommand = f"perl /home/Vextorite/Documents/Capstone/ssrg-ndxsas021-hlnsan005-rmrsuv002/backend/SSRG/Jobs/MossBackendJobs/mossnet.pl -l {self.submissionLanguage} -d {subPathList} > myMossRun.txt"
        if(self.directoryFormat==False) and (len(self.baseFile)==0):
            osCommand = f"perl /home/Vextorite/Documents/Capstone/ssrg-ndxsas021-hlnsan005-rmrsuv002/backend/SSRG/Jobs/MossBackendJobs/mossnet.pl -l {self.submissionLanguage} {subPathList} > myMossRun.txt"
        print(osCommand)
        return osCommand
...
job = jobRequest(str(sys.argv[1]), str(sys.argv[2]), str(sys.argv[3]), eval(str(sys.argv[4])), str(sys.argv[5]), str(sys.argv[6]), eval(str(sys.argv[7])), str(sys.argv[8]))
